...
class DatabaseManager:
    def __init__(self):
        ...
        if self.is_postgres and not asyncpg:
            logger.error(
                "PostgreSQL URL provided but asyncpg is not installed!"
            )

    async def _get_pool(self):
        """
        Lazily create and return the PostgreSQL connection pool.

        statement_cache_size=0 is important for environments such as
        PgBouncer/Supabase transaction pooling where prepared statement
...
async def search_chunks(
    *,
    ...
    try:
        ...
        logger.warning(
            "DB search failed (table may be empty): %s",
            exc,
        )
        return []
...
async def delete_conversation_row(
    id: str,
) -> None:
    await db_manager.execute(
